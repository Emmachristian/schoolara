...
Architecture:
- Regular views (views.py)  — full page loads, HTMX list/filter, action endpoints
- Modal views (modal_views.py) — HTMX partial content for modals

Result-entry flow:
  results/by-class/                           → class_results_selector   (pick session + class)
  classes/<pk>/results/                       → class_results_dashboard  (read-only overview / list)
  classes/<pk>/results/entry/<category_pk>/  → class_category_results_entry (score-entry grid)
  examinations/<pk>/results/bulk-entry/       → bulk_result_entry        (single-exam quick entry)
"""
...
urlpatterns = [
    ...
    # =========================================================================
    # GRADING RANGES  (modal-only — CRUD is handled via the GradingSystem formset)
    # =========================================================================
    ...
    # --- Selector: pick session + class ---
    path('results/by-class/', views.class_results_selector, name='results_by_class'),

    # --- Global results list (with filtering) ---
    path('results/', views.result_list, name='result_list'),

    # --- Class overview (read-only dashboard + list mode) ---
    # ?mode=dashboard  → category tabs with subject columns
    # ?mode=list       → paginated results table with filters
    path('classes/<uuid:class_pk>/results/', views.class_results_dashboard, name='class_results_dashboard'),

    # --- Score-entry grid: students (rows) × subjects in one category (columns) ---
    path(
        'classes/<uuid:class_pk>/results/entry/<uuid:category_pk>/',
        views.class_category_results_entry,
        name='class_category_results_entry',
    ),
    ...
    # --- Single-exam bulk entry (from examination detail page) ---
    path(
        'examinations/<uuid:examination_pk>/results/bulk-entry/',
        views.bulk_result_entry,
        name='bulk_result_entry',
    ),
    ...
    path('ajax/students-for-examination/<uuid:examination_pk>/', views.ajax_get_students_for_examination, name='ajax_get_students_for_examination'),
