...
from django.db.models import (
    ...
    Subquery, OuterRef, Exists, ExpressionWrapper,
)
...
def get_student_account_statistics(filters=None):
    ...
    if account_ids:
        transaction_totals = AccountTransaction.objects.filter(
            ...
            # Sum of all signed amounts = net balance across all accounts
    ...
    stats['balances'] = {
        ...
        'total_fees_charged':       float(abs(transaction_totals['total_charges'])),
        ...
        'total_refunds_issued':     float(abs(transaction_totals['total_refunds'])),
    }

    accounts_with_balance = accounts.annotate(
        calculated_balance=Coalesce(Sum('transactions__amount'), Decimal('0.00'))
    )
    ...
    debt_totals = debtors.aggregate(
        ...
        max_debt=Coalesce(Min('calculated_balance'), Decimal('0.00')),
    )
    ...
    stats['debt_analysis'] = {
        ...
        'total_outstanding':     float(abs(debt_totals['total_debt'])),
        'average_debt':          float(abs(debt_totals['avg_debt'])),
        'largest_debt':          float(abs(debt_totals['max_debt'])),
    ...
    stats['collection_rate'] = round(
        float(total_paid / total_charged * 100) if total_charged > 0 else 0,
        2
    )
    ...
    if accounts_with_limit.exists():
        ...
        over_limit_count = sum(
            1 for acc in accounts_with_limit
    ...
    now = timezone.now()
    stats['activity'] = {
        'transactions_last_7_days':  accounts.filter(last_transaction_date__gte=now - timedelta(days=7)).count(),
        'transactions_last_30_days': accounts.filter(last_transaction_date__gte=now - timedelta(days=30)).count(),
        'payments_last_7_days':      accounts.filter(last_payment_date__gte=now - timedelta(days=7)).count(),
        'payments_last_30_days':     accounts.filter(last_payment_date__gte=now - timedelta(days=30)).count(),
...
def get_account_transaction_statistics(filters=None):
    ...
    if filters:
        ...
        if filters.get('academic_session'):
            transactions = transactions.filter(academic_session_id=filters['academic_session'])
        if filters.get('fiscal_period'):
            transactions = transactions.filter(fiscal_period_id=filters['fiscal_period'])
    ...
    stats['by_type'] = {
        item['transaction_type']: {
            ...
            'percentage':     round(
                (item['count'] / total_transactions * 100) if total_transactions > 0 else 0, 2
            ),
        }
    ...
    stats['financial_totals'] = {
        ...
        'net_amount':        float(financial_totals['total_credits'] - financial_totals['total_debits']),
    }
    ...
    # Session breakdown — use session name field
    session_stats = transactions.filter(
        academic_session__isnull=False
    ).values('academic_session__year_name', 'academic_session__term_name').annotate(
        count=Count('id'),
        total_amount=Coalesce(Sum('amount'), Decimal('0.00')),
    ).order_by('-count')[:10]
    ...
    stats['by_session'] = [
        {
            'session':      (f"{item['academic_session__year_name']} - {item['academic_session__term_name']}" if item.get('academic_session__year_name') else 'Unknown'),
            'count':        item['count'],
            'total_amount': float(item['total_amount']),
        }
    ...
    stats['most_active_accounts'] = [
        {
            'student_name':     (
                f"{item['student_account__student__first_name']} "
                f"{item['student_account__student__last_name']}"
            ),
    ...
    stats['recent_activity'] = {
        'last_24_hours': transactions.filter(created_at__gte=now - timedelta(hours=24)).count(),
        'last_7_days':   transactions.filter(created_at__gte=now - timedelta(days=7)).count(),
        'last_30_days':  transactions.filter(created_at__gte=now - timedelta(days=30)).count(),
    }
...
def get_fee_structure_statistics(filters=None):
    ...
    """
    from .models import FeesStructure, FeesStructureItem

    structures = FeesStructure.objects.prefetch_related(
        'academic_levels', 'applicable_classes',
        'applicable_sessions', 'items', 'items__fee_category',
    )
    ...
    if filters:
        if filters.get('structure_type'):
            structures = structures.filter(structure_type=filters['structure_type'])
        if filters.get('boarding_type_filter'):
            structures = structures.filter(boarding_type_filter=filters['boarding_type_filter'])
        if filters.get('student_type_filter'):
            structures = structures.filter(student_type_filter=filters['student_type_filter'])
        if filters.get('is_active') is not None:
            structures = structures.filter(is_active=filters['is_active'])
        if filters.get('academic_session'):
            structures = structures.filter(applicable_sessions__id=filters['academic_session'])
        if filters.get('academic_level'):
            structures = structures.filter(academic_levels__id=filters['academic_level'])
        if filters.get('has_expired'):
            structures = structures.filter(expiry_date__lt=today)
    ...
    stats['status_breakdown'] = {
        ...
        'not_yet_effective': structures.filter(is_active=True, effective_date__gt=today).count(),
        'expired':           structures.filter(expiry_date__lt=today).count(),
    ...
    stats['by_boarding_type'] = [
        {
            ...
            'count':         item['count'],
            'percentage':    round(
                (item['count'] / total_structures * 100) if total_structures > 0 else 0, 2
            ),
        }
    ...
    # Use ExpressionWrapper to avoid DecimalField arithmetic issues
    structure_items = FeesStructureItem.objects.filter(fee_structure__in=structures)
    ...
    discount_expr = ExpressionWrapper(
        F('amount') * F('default_discount_percentage') / 100,
        output_field=DecimalField(max_digits=12, decimal_places=2),
    )
    ...
    financial_stats = structure_items.aggregate(
        ...
        total_tax=Coalesce(Sum(tax_expr), Decimal('0.00')),
        total_discount=Coalesce(Sum(discount_expr), Decimal('0.00')),
    )
    ...
    stats['top_structures_by_fees'] = [
        {
            ...
            'boarding_type': s.boarding_type_filter,
    ...
    stats['usage_statistics'] = {
        ...
        'collection_rate':             round(
            float(usage_stats['total_paid'] / usage_stats['total_billed'] * 100)
    ...
    most_used = structures.annotate(
        invoice_count=Count('invoices')
    ).filter(invoice_count__gt=0).order_by('-invoice_count')[:10]
    ...
    scholarship_eligible_items = structure_items.filter(scholarship_eligible=True).count()
    ...
    stats['scholarship_configuration'] = {
        ...
        'non_eligible_items':         total_items - scholarship_eligible_items,
        'eligibility_rate':           round(
            (scholarship_eligible_items / total_items * 100) if total_items > 0 else 0, 2
        ),
    }
    ...
    stats['conditional_items'] = {
        ...
        'unconditional_items':     total_items - conditional_items,
        'conditional_rate':        round(
            (conditional_items / total_items * 100) if total_items > 0 else 0, 2
        ),
    }

    # Installment configuration
    installment_stats = structure_items.filter(
...
def get_invoice_statistics(filters=None):
    ...
    # Renamed output keys to avoid shadowing field names used in
    # later aggregates (e.g. Avg('total_amount') would resolve to
    # the output key instead of the model field if named the same).
    financial_totals = invoices.aggregate(
        sum_total_amount=Coalesce(Sum('total_amount'), Decimal('0.00')),
        sum_subtotal=Coalesce(Sum('subtotal_amount'), Decimal('0.00')),
        sum_discounts=Coalesce(Sum('discount_amount'), Decimal('0.00')),
        sum_scholarship_discounts=Coalesce(Sum('scholarship_discount_amount'), Decimal('0.00')),
        sum_tax=Coalesce(Sum('tax_amount'), Decimal('0.00')),
        sum_paid=Coalesce(Sum('paid_amount'), Decimal('0.00')),
        sum_balance=Coalesce(Sum('balance'), Decimal('0.00')),
        sum_late_fees=Coalesce(Sum('late_fee_amount'), Decimal('0.00')),
        avg_invoice_amount=Coalesce(Avg('total_amount'), Decimal('0.00')),
    )

    stats['financial_totals'] = {
        'total_billed':                float(financial_totals['sum_total_amount']),
        'subtotal':                    float(financial_totals['sum_subtotal']),
        'total_discounts':             float(financial_totals['sum_discounts']),
        'total_scholarship_discounts': float(financial_totals['sum_scholarship_discounts']),
        'total_tax':                   float(financial_totals['sum_tax']),
        'total_paid':                  float(financial_totals['sum_paid']),
        'total_outstanding':           float(financial_totals['sum_balance']),
        'total_late_fees':             float(financial_totals['sum_late_fees']),
        'average_invoice':             float(financial_totals['avg_invoice_amount']),
    }

    stats['payment_rate'] = round(
        float(financial_totals['sum_paid'] / financial_totals['sum_total_amount'] * 100)
        if financial_totals['sum_total_amount'] > 0 else 0,
        2
    )
    ...
    stats['overdue'] = {
        ...
        'percentage_of_total': round(
            (overdue_stats['count'] / total_invoices * 100) if total_invoices > 0 else 0, 2
        ),
    }
    ...
    aging_ranges = [
        ...
        ('31_60_days', Q(due_date__lt=today - timedelta(days=30), due_date__gte=today - timedelta(days=60))),
        ('61_90_days', Q(due_date__lt=today - timedelta(days=60), due_date__gte=today - timedelta(days=90))),
    ...
    for label, condition in aging_ranges:
        aging_data = invoices.filter(
            condition, status__in=['PENDING', 'PARTIALLY_PAID', 'OVERDUE']
        ).aggregate(
            count=Count('id'),
    ...
    stats['by_session'] = [
        {
            'session':      (
                f"{item['academic_session__year_name']} - {item['academic_session__term_name']}"
                if item.get('academic_session__year_name') else 'Unknown'
            ),
            'count':        item['count'],
            'total_amount': float(item['total_amount']),
    ...
    structure_stats = invoices.values(
        'fee_structure__name', 'fee_structure__structure_type'
    ).annotate(
        count=Count('id'),
        total_amount=Coalesce(Sum('total_amount'), Decimal('0.00')),
    ...
    now = timezone.now()
    stats['recent_activity'] = {
        'created_last_7_days':  invoices.filter(created_at__gte=now - timedelta(days=7)).count(),
        'created_last_30_days': invoices.filter(created_at__gte=now - timedelta(days=30)).count(),
        'paid_last_7_days':     invoices.filter(status='PAID', updated_at__gte=now - timedelta(days=7)).count(),
        'paid_last_30_days':    invoices.filter(status='PAID', updated_at__gte=now - timedelta(days=30)).count(),
    }
...
def get_payment_statistics(filters=None):
    ...
    status_breakdown = payments.values('status').annotate(
        count=Count('id'),
        total_amount=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')),
    ).order_by('-count')
    ...
    stats['by_payment_method'] = [
        {
            ...
            'average_amount': float(item['avg_amount']),
            'percentage':     round(
                (item['count'] / total_payments * 100) if total_payments > 0 else 0, 2
            ),
        }
    ...
    stats['verification'] = {
        ...
        'verified_amount':   float(
            verified.aggregate(total=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')))['total']
        ),
        'unverified_count':  unverified.count(),
        'unverified_amount': float(
            unverified.aggregate(total=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')))['total']
        ),
        'verification_rate': round(
            (verified.count() / total_payments * 100) if total_payments > 0 else 0, 2
        ),
    }

    # Reversal and refund tracking
    reversed_payments = payments.filter(reversed=True)
    refunded_payments = payments.filter(refunded=True)

    stats['reversals_and_refunds'] = {
        'reversed_count':  reversed_payments.count(),
        'reversed_amount': float(
            reversed_payments.aggregate(total=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')))['total']
        ),
        'refunded_count':  refunded_payments.count(),
        'refunded_amount': float(
            refunded_payments.aggregate(total=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')))['total']
        ),
    }
    ...
    stats['receipts'] = {
        ...
        'issue_rate': round(
            (with_receipt.count() / total_payments * 100) if total_payments > 0 else 0, 2
        ),
    }

    # Time-based analysis
    if payments.exists():
        thirty_days_ago = get_school_today() - timedelta(days=30)
        daily_payments = payments.filter(
            ...
            count=Count('id'),
            total=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')),
        ).order_by('day')
    ...
    # Session breakdown — use session name
    session_stats = payments.filter(
        academic_session__isnull=False
    ).values('academic_session__year_name', 'academic_session__term_name').annotate(
        count=Count('id'),
        total_amount=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')),
    ...
    top_payers = payments.values(
        'student__id', 'student__first_name',
        'student__last_name', 'student__admission_number',
    ).annotate(
        payment_count=Count('id'),
        total_paid=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')),
    ...
    now = timezone.now()
    stats['recent_activity'] = {
        'payments_last_24_hours': payments.filter(created_at__gte=now - timedelta(hours=24)).count(),
        'payments_last_7_days':   payments.filter(created_at__gte=now - timedelta(days=7)).count(),
        'payments_last_30_days':  payments.filter(created_at__gte=now - timedelta(days=30)).count(),
        'amount_last_24_hours':   float(
            payments.filter(
                created_at__gte=now - timedelta(hours=24)
            ).aggregate(total=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')))['total']
        ),
    }
...
def get_scholarship_statistics(filters=None):
    ...
    scholarships = StudentScholarship.objects.select_related('student', 'scholarship_program')

    if filters:
        if filters.get('program_id'):
            scholarships = scholarships.filter(scholarship_program_id=filters['program_id'])
        if filters.get('status'):
    ...
    # Program discount type breakdown
    discount_type_stats = programs.values('discount_type').annotate(
        count=Count('id')
    ).order_by('-count')

    stats['programs']['by_discount_type'] = [
        {'discount_type': item['discount_type'], 'count': item['count']}
        for item in discount_type_stats
    ]

    # Category-specific vs global
    stats['programs']['category_specific_count'] = programs.filter(
        discount_type='CATEGORY_SPECIFIC'
    ).count()
    stats['programs']['global_discount_count'] = programs.exclude(
        discount_type='CATEGORY_SPECIFIC'
    ).count()
    ...
    stats['budget'] = {
        ...
        'remaining':        float(budget_stats['total_budget'] - budget_stats['total_used']),
        'utilization_rate': round(
            float(budget_stats['total_used'] / budget_stats['total_budget'] * 100)
    ...
    stats['applications'] = {
        ...
        'by_status': {item['status']: item['count'] for item in application_status_stats},
    }
    ...
    top_programs = programs.annotate(
        ...
        total_disbursed=Coalesce(
            Sum('student_scholarships__total_amount_used'), Decimal('0.00')
        ),
    ...
    # Impact tracking via ScholarshipApplicationLog
    application_logs = ScholarshipApplicationLog.objects.filter(is_reversed=False)
...
# =============================================================================
# DISCOUNT STATISTICS  (uses DiscountPolicy / StudentDiscount / DiscountApplication)
# =============================================================================
...
def get_discount_statistics(filters=None):
    ...
    Args:
        filters (dict): Optional filters
            - category: Filter by DiscountPolicy.category
            - value_mode: Filter by value_mode
            - is_active: Filter by active status
            - academic_session: Filter by valid_sessions membership

    Returns:
        dict: Discount statistics
    """
    from .models import DiscountPolicy, StudentDiscount, DiscountApplication

    policies = DiscountPolicy.objects.all()

    if filters:
        if filters.get('category'):
            policies = policies.filter(category=filters['category'])
        if filters.get('value_mode'):
            policies = policies.filter(value_mode=filters['value_mode'])
        if filters.get('is_active') is not None:
            policies = policies.filter(is_active=filters['is_active'])
        if filters.get('academic_session'):
            policies = policies.filter(valid_sessions__id=filters['academic_session'])
    ...
    stats = {
        ...
        'inactive_policies': policies.filter(is_active=False).count(),
        'auto_apply_count':  policies.filter(auto_apply=True).count(),
    }

    # By discount category
    category_stats = policies.values('category').annotate(
        count=Count('id'),
        active_count=Count('id', filter=Q(is_active=True)),
    ).order_by('-count')
    ...
    # By value mode
    value_mode_stats = policies.values('value_mode').annotate(
        count=Count('id'),
        avg_flat_percentage=Coalesce(Avg('flat_percentage'), Decimal('0.00')),
    ).order_by('-count')
    ...
    stats['by_value_mode'] = [
        {
            ...
            'count':                item['count'],
            'avg_flat_percentage':  float(item['avg_flat_percentage']),
        }
        for item in value_mode_stats
    ]

    # By combination mode
    combination_stats = policies.values('combination_mode').annotate(
        count=Count('id')
    ).order_by('-count')

    stats['by_combination_mode'] = [
        {'combination_mode': item['combination_mode'], 'count': item['count']}
        for item in combination_stats
    ]

    # By application method
    application_method_stats = policies.values('application_method').annotate(
        count=Count('id')
    ).order_by('-count')

    stats['by_application_method'] = [
        {'application_method': item['application_method'], 'count': item['count']}
        for item in application_method_stats
    ]

    # Budget tracking
    with_budget  = policies.filter(total_budget__isnull=False)
    budget_stats = with_budget.aggregate(
        total_budget=Coalesce(Sum('total_budget'), Decimal('0.00')),
        total_used=Coalesce(Sum('budget_used'), Decimal('0.00')),
    )

    stats['budget'] = {
        'policies_with_budget':      with_budget.count(),
        'policies_without_budget':   total_policies - with_budget.count(),
    ...
    student_discounts = StudentDiscount.objects.all()

    student_discount_stats = student_discounts.aggregate(
        total=Count('id'),
    ...
    stats['student_discount_awards'] = {
        'total':     student_discount_stats['total'],
        'active':    student_discount_stats['active'],
        'suspended': student_discount_stats['suspended'],
        'expired':   student_discount_stats['expired'],
        'revoked':   student_discount_stats['revoked'],
        'pending':   student_discount_stats['pending'],
    }

    # Discount applications (audit records)
    applications = DiscountApplication.objects.filter(is_reversed=False)

    app_stats = applications.aggregate(
        total_applications=Count('id'),
        total_value=Coalesce(Sum('amount_discounted'), Decimal('0.00')),
        avg_discount=Coalesce(Avg('amount_discounted'), Decimal('0.00')),
        unique_students=Count('student_discount__student', distinct=True),
        unique_invoices=Count('invoice', distinct=True),
    )
    ...
    stats['applications'] = {
        ...
        'invoices_affected':    app_stats['unique_invoices'],
    }

    # Top policies by usage (applications count)
    top_policies = policies.annotate(
        application_count=Count('student_awards__applications'),
        total_discounted=Coalesce(
            Sum('student_awards__applications__amount_discounted'), Decimal('0.00')
        ),
    ...
    stats['top_policies'] = [
        {
            ...
            'total_discounted':  float(p.total_discounted),
    ...
    # Priority distribution
    priority_stats = policies.values('priority').annotate(
        count=Count('id')
    ).order_by('priority')[:10]
...
# =============================================================================
# REFUND STATISTICS  (uses Payment.refunded=True — no separate Refund model)
# =============================================================================

def get_refund_statistics(filters=None):
    """
    Get comprehensive refund statistics.
    Refunds are tracked on the Payment model via Payment.refunded = True.

    Args:
        filters (dict): Optional filters
            - refund_method: Filter by refund_method
            - academic_session: Filter by session ID
            - fiscal_period: Filter by fiscal period ID
            - date_from: Start date filter (refunded_on)
            - date_to: End date filter (refunded_on)
    ...
    if filters:
        if filters.get('refund_method'):
            refunds = refunds.filter(refund_method=filters['refund_method'])
        if filters.get('academic_session'):
        ...
        if filters.get('date_from'):
            refunds = refunds.filter(refunded_on__date__gte=filters['date_from'])
        if filters.get('date_to'):
            refunds = refunds.filter(refunded_on__date__lte=filters['date_to'])

    total_refunds = refunds.count()

    stats = {'total_refunds': total_refunds}

    # Financial totals
    financial_totals = refunds.aggregate(
        total_refunded=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')),
        avg_refund=Coalesce(Avg('amount'), Decimal('0.00')),
        max_refund=Coalesce(Max('amount'), Decimal('0.00')),
        min_refund=Coalesce(Min('amount'), Decimal('0.00')),
    )

    stats['financial_totals'] = {
        'total_refunded': float(financial_totals['total_refunded']),
        'average_refund': float(financial_totals['avg_refund']),
        'largest_refund': float(financial_totals['max_refund']),
        'smallest_refund': float(financial_totals['min_refund']),
    }

    # By refund method
    method_stats = refunds.values('refund_method').annotate(
        count=Count('id'),
        total_amount=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')),
    ).order_by('-total_amount')

    stats['by_refund_method'] = [
        {
            'method':       item['refund_method'] or 'Not Specified',
            'count':        item['count'],
            'total_amount': float(item['total_amount']),
            'percentage':   round(
                (item['count'] / total_refunds * 100) if total_refunds > 0 else 0, 2
            ),
        }
        for item in method_stats
    ]

    # By original payment method
    orig_method_stats = refunds.values(
    ...
    stats['by_original_payment_method'] = [
        {
            'method':       item['payment_method__name'],
            'count':        item['count'],
            'total_amount': float(item['total_amount']),
        }
        for item in orig_method_stats
    ]
    ...
    # Time trends
    if refunds.filter(refunded_on__isnull=False).exists():
        monthly_refunds = refunds.filter(
            refunded_on__isnull=False
        ).annotate(
            month=TruncMonth('refunded_on')
        ).values('month').annotate(
            count=Count('id'),
            total=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')),
        ).order_by('-month')[:12]
        ...
        stats['monthly_trends'] = [
            ...
            for item in monthly_refunds
        ]

    # Processing time (payment_date → refunded_on) at DB level
    # We use annotation to get days difference where refunded_on is set
    now = timezone.now()
    recent_refunds = refunds.filter(refunded_on__isnull=False)

    if recent_refunds.exists():
        # Calculate in Python from queryset — DB-agnostic approach
        processing_days = []
        for r in recent_refunds.values('payment_date', 'refunded_on')[:500]:
            if r['payment_date'] and r['refunded_on']:
                refunded_date = (
                    r['refunded_on'].date()
                    if hasattr(r['refunded_on'], 'date')
                    else r['refunded_on']
                )
                days = (refunded_date - r['payment_date']).days
                if days >= 0:
                    processing_days.append(days)

        if processing_days:
            stats['processing_metrics'] = {
                'avg_processing_days': round(sum(processing_days) / len(processing_days), 1),
                'min_processing_days': min(processing_days),
                'max_processing_days': max(processing_days),
                'sample_size':         len(processing_days),
            }

    # Top students with refunds
    top_students = refunds.values(
        'student__id', 'student__first_name',
        'student__last_name', 'student__admission_number',
    ).annotate(
        refund_count=Count('id'),
        total_refunded=Coalesce(Sum('amount_in_school_currency'), Decimal('0.00')),
    ).order_by('-total_refunded')[:10]

    stats['top_refund_students'] = [
        {
            'student_id':       str(item['student__id']),
            'student_name':     f"{item['student__first_name']} {item['student__last_name']}",
            'admission_number': item['student__admission_number'],
            'refund_count':     item['refund_count'],
            'total_refunded':   float(item['total_refunded']),
        }
    ...
    # Recent activity
    stats['recent_activity'] = {
        'refunds_last_7_days':  refunds.filter(
            refunded_on__gte=now - timedelta(days=7)
        ).count(),
        'refunds_last_30_days': refunds.filter(
            refunded_on__gte=now - timedelta(days=30)
        ).count(),
...
def get_display_group_statistics(filters=None):
    ...
    Args:
        filters (dict): Optional filters
            - is_active: Filter by active status
            - show_as_group: Filter by grouping behaviour
    ...
    stats['grouping_behavior'] = {
        ...
        'show_subtotal':       groups.filter(show_as_group=True, show_group_subtotal=True).count(),
        'hide_subtotal':       groups.filter(show_as_group=True, show_group_subtotal=False).count(),
    }

    # Categories per group
    groups_with_categories = groups.annotate(category_count=Count('feescategory'))
    ...
    stats['groups_by_size'] = [
        {'category_count': item['category_count'], 'group_count': item['count']}
    ...
    stats['largest_groups'] = [
        {
            'group_id':       str(g.id),
            'name':           g.name,
            'category_count': g.category_count,
            'display_order':  g.display_order,
            'show_as_group':  g.show_as_group,
    ...
    # Categories without a display group
    stats['ungrouped_categories'] = {
        'count': FeesCategory.objects.filter(display_group__isnull=True).count(),
    }
    ...
    stats['color_distribution'] = [
        {'color_code': item['color_code'], 'usage_count': item['count']}
        for item in color_usage[:10]
    ]
    ...
    # Ordering conflicts (duplicate display_order values)
    ...
    stats['ordering_conflicts'] = {
        ...
        'duplicate_orders': [
            {'display_order': item['display_order'], 'group_count': item['count']}
            for item in duplicate_orders
        ],
    }

    # Active vs inactive
    active_groups_qs   = groups.filter(is_active=True)
    inactive_groups_qs = groups.filter(is_active=False)

    stats['active_vs_inactive'] = {
        'active': {
            'count':             active_groups_qs.count(),
            'total_categories':  active_groups_qs.aggregate(
                t=Count('feescategory')
            )['t'],
        },
        'inactive': {
            'count':             inactive_groups_qs.count(),
            'total_categories':  inactive_groups_qs.aggregate(
                t=Count('feescategory')
            )['t'],
        },
    }
    ...
    stats['configuration_summary'] = {
        'total_active_with_categories':   groups.filter(
            is_active=True
        ).annotate(cc=Count('feescategory')).filter(cc__gt=0).count(),
        'total_inactive_with_categories': groups.filter(
            is_active=False
        ).annotate(cc=Count('feescategory')).filter(cc__gt=0).count(),
        'showing_as_groups':              groups.filter(is_active=True, show_as_group=True).count(),
        'showing_individually':           groups.filter(is_active=True, show_as_group=False).count(),
    }
...
def get_fee_category_statistics(filters=None):
    ...
    stats['by_type'] = [
        {'type': item['category_type'], 'count': item['count']}
        for item in type_stats
    ]
    ...
    stats['by_applicability'] = [
        {'applicability': item['applicability'], 'count': item['count']}
        for item in applicability_stats
    ]

    # By frequency
    frequency_stats = categories.values('frequency').annotate(
        count=Count('id')
    ).order_by('-count')
    ...
    stats['configuration'] = {
        ...
        'partial_payment_allowed':   categories.filter(allows_partial_payment=True).count(),
        'recurring':                 categories.filter(is_recurring=True).count(),
        'one_time':                  categories.filter(is_recurring=False).count(),
    }

    # Revenue by category (from invoice items)
    revenue_stats = FeeInvoiceItem.objects.values(
        'fee_category__name', 'fee_category__category_type', 'fee_category__code',
    ).annotate(
    ...
    # Categories not used in any active structure
    from .models import FeesStructureItem
    used_category_ids = set(
        FeesStructureItem.objects.filter(
            fee_structure__is_active=True
        ).values_list('fee_category_id', flat=True)
    )

    stats['unused_in_active_structures'] = categories.filter(
        is_active=True
    ).exclude(id__in=used_category_ids).count()
...
def get_financial_dashboard(academic_session_id=None, fiscal_period_id=None):
    ...
    dashboard['financial_overview'] = {
        ...
        'collection_rate':   round(
            float(payment_total['total'] / financial['total_billed'] * 100)
